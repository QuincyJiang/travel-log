...
全屏查看器支持按钮、鼠标滚轮、键盘 `+/-` 和双击缩放；放大后可拖动查看细节，切换照片时恢复为 100%。

每张照片可在灯箱或批量选择工具栏中加入精选。每段旅程最多精选 16 张；相册默认打开精选集，并使用保留留白的不等距编辑网格。登录 Cloudflare Access 后，首页旅程封面会静默替换为精选照片马赛克；未登录时仍显示默认旅程封面。
